...
#   当時はベースラインの失敗が 1 件あった
#   (NetworkHistoryService_ConcurrentWrites_ThreadSafe — テスト間の状態共有バグ)。
#   これは同セッション内で保存先を注入可能にする API 変更により修正済みで、
#   それ以降ベースラインは 0 件(このスクリプト自身が毎回実測して表示する。
#   具体的な失敗数はここに固定値として書き写さない — 実行時出力が単一の真実の源)。
#
