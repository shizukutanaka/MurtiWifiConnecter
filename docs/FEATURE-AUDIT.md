...
| `Hotspot20Service` | Passpoint / キャリア Wi-Fi | **ブロッカーの Core 側は解消(2026-07 第4パス)** — 802.11u Interworking 要素 (ID 107) の検出を `BeaconIeParser.HasInterworking` に実装し、`BeaconIeApplier` 経由で `BssInfo.HasInterworkingElement` へ配線した。**残るはプラットフォーム層による生 IE バイトの供給のみ**(`IBeaconIeProvider` の Windows 実装)。テスト: `tests/MWC.Core.Tests/InterworkingIeTests.cs` | 日本キャリア(au/SoftBank/docomo)プリセット付き。2026-H2 追補: OpenRoaming が主流化中(WBA 2025 調査で回答企業の81%が導入計画)で配線価値は上昇傾向だが、ブロッカー(802.11u Interworking IE 抽出未実装)は不変 |
