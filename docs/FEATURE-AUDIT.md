...
| 8 | `SECURITY.md` が未実施の署名・SLSA・SBOM を現在形で宣言 | ①要件を疑う → ⑤自動化 | 主張を条件付きに是正し、`docs/ci/release.yml` を用意。守衛が無条件主張を拒否する |
| 9 | Dependabot が空のエコシステムを監視し、設定キーも無効 | ②削除 → ⑤自動化 | `automerge:`(スキーマ外)を削除し nuget を対象化。**本セッションで push 済み** |
| 10 | README が WCAG AAA を無条件に主張(実際は本文 4 テーマのみ、アクセントは全テーマ AA) | ①要件を疑う | 範囲を明記。守衛が全称表現を拒否する |
...
4. **調べて健全だった箇所も記録する(再調査の重複を防ぐため)。**
   本パスで疑って**問題が無かった**もの: `mwc scan --evil-twin` の出力
   (「ステートレスなので 4 つのうち 1 つの判定しか働かない」旨を既に明示していた)、
   `NetworkDetailViewModel` の `HasMlo` 以外の `Has*` フラグ
   (いずれもデータ不在を正しく区別している。特に `HasLinkEstimate` は
   PHY モデルが妥当な場合のみ表示し、過大推定を避けている)、
   キャプティブポータル(`CaptivePortalDialog` は接続フローから起動されており配線済み)、
   `docs/*.md` が参照する `mwc <cmd>` の実在性、resx キーの双方向到達性。
5. **「可能」と「実施している」は違う。** `oui-update.ps1` は自身のコメントで
