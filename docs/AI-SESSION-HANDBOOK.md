...
| i18n 体制 | 名前付き 14 ロケール + 中立ベースの計 15 resx ファイル × 520 キー(README バッジの「14 langs」はこの名前付きロケール数で正しい)。`LocaleKeyConsistencyTests` がキー欠落を自動検出。UI 文字列は必ず resx 経由(CLAUDE.md 必須) |
