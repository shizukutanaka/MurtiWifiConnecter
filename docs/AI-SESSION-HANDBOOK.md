...
> 分離してある。そこに載っている 4 件のうち、CI 設置・Release・MLO リンク詳細の 3 件は
> AI セッションが再挑戦しても同じ壁に当たる。残る MAC 自動取得は 2026-08 にモデルと CLI 側の
> 配線を Core へ切り出せたため、Windows での実アドレス供給という最後の 1 箇所を除いて解消済み
> (詳細は `docs/COMPLETION-CHECKLIST.md` §4)。
...
| i18n 体制 | 名前付き 14 ロケール + 中立ベースの計 15 resx ファイル × 517 キー(README バッジの「14 langs」はこの名前付きロケール数で正しい)。`LocaleKeyConsistencyTests` がキー欠落を自動検出。UI 文字列は必ず resx 経由(CLAUDE.md 必須) |
...
| MAC ランダム化検出 | **✅ 分解できた**(2026-08 に判定を訂正) | 当初「OS の設定値なので不可」としたが**問いの取り違え**だった。要るのは設定ではなく**効果**で、それは MAC の LAA ビットに現れる。解析すべきバイト列は存在した(`MacAddressModeInference`) |

**第 3 の軸(2026-08 追加。上の表の 3 行目を実際に間違えたので明文化する):**

> **設定そのものは読めなくても、その効果が観測値に現れるなら Core で判定できる。**

「OS の設定値だから無理」は結論ではなく、**まだ問いが足りていない状態**である。
正しい問いは「その設定が有効なとき、観測できる値の何が変わるか」。
MAC ランダム化の場合、変わるのはアドレスの Locally Administered ビットで、
これは純粋なバイト判定 = Core の仕事だった。

新しい「要プラットフォーム」項目に当たったら、順に問うこと:

1. その値はビーコンで**広告される**か → Core で解析できる
2. **実測値**か → 実機が要る
3. 実測値に見えても、**設定の効果が別の観測値に現れないか** → Core に戻せることがある
